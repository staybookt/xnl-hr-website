          ...
          <Container size="wide">
            <motion.div initial="hidden" animate="show" variants={STAGGER} className="max-w-5xl">
              ...
              <motion.div variants={FADE_UP} className="flex flex-col sm:flex-row gap-4 sm:gap-5 items-start sm:items-center">
                <Link href="/contact?topic=hrto" className="inline-flex items-center justify-center gap-2 rounded-full bg-[var(--color-brand)] text-white px-8 py-4 text-[16px] font-semibold hover:bg-[var(--color-brand-deep)] transition-colors">
              ...
              {/* Guarantee badge — stacks on mobile to keep 'Mediation-ready in 10 business days' from overflowing iPhone SE */}
              <motion.div variants={FADE_UP} className="mt-10 md:mt-16 inline-flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-4 rounded-2xl bg-white/10 backdrop-blur-md border border-white/15 px-5 py-4 sm:px-6 max-w-full">
                <span className="text-[10px] uppercase tracking-[0.18em] font-semibold text-[var(--color-brand)] flex-shrink-0">Guarantee</span>
                <span className="text-[14px] md:text-[15px] font-semibold text-white leading-[1.4]">Mediation-ready in 10 business days from intake.</span>
              </motion.div>
            </motion.div>
          </Container>
        </div>
      ...
      <section className="py-20 md:py-32 bg-[var(--color-paper)]">
        <Container size="wide">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-16 items-start">
            <motion.div initial={{ opacity: 1, scale: 0.97 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true, margin: "-50px" }} transition={{ duration: 1, ease: SLOW_OUT }} className="lg:col-span-4 max-w-[380px] mx-auto lg:mx-0">
              <PortraitWendy className="w-full h-auto" />
              <div className="flex items-center justify-between mt-3 text-[13px] text-[var(--color-mute)]">
            ...
            <motion.div initial={{ opacity: 1, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-50px" }} transition={{ duration: 0.8, ease: SLOW_OUT, delay: 0.1 }} className="lg:col-span-8">
              ...
              <div className="space-y-5 text-[16px] md:text-[18px] leading-[1.65] text-[var(--color-ink-soft)] mt-2">
        ...
        <Container size="wide">
          <div className="grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-16 items-start">
            <div className="md:col-span-4">
              ...
              <h3 className="text-[26px] md:text-[32px] leading-[1.15] tracking-[-0.02em] font-semibold text-[var(--color-ink)]">Per-session pricing.</h3>
            </div>
            <div className="md:col-span-8 grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6">
              <div className="rounded-2xl bg-[var(--color-paper)] border border-[var(--color-rule)] p-5 sm:p-6">
                <p className="text-[11px] uppercase tracking-[0.14em] font-semibold text-[var(--color-mute)] mb-2">Half-day</p>
                <p className="text-[26px] sm:text-[28px] font-semibold text-[var(--color-ink)] mb-1">{mediationService.price}</p>
                <p className="text-[12px] text-[var(--color-mute)] inline-flex flex-wrap items-center gap-1.5">Per session<span className="text-[9px] uppercase tracking-[0.14em] font-semibold text-[var(--color-brand-deep)] bg-[var(--color-brand-soft)] px-2 py-0.5 rounded-full">Illustrative</span></p>
              </div>
              <div className="rounded-2xl bg-[var(--color-paper)] border border-[var(--color-rule)] p-5 sm:p-6">
                <p className="text-[11px] uppercase tracking-[0.14em] font-semibold text-[var(--color-mute)] mb-2">Full-day</p>
                <p className="text-[26px] sm:text-[28px] font-semibold text-[var(--color-ink)] mb-1">From $3,000</p>
                <p className="text-[12px] text-[var(--color-mute)] inline-flex flex-wrap items-center gap-1.5">Per session<span className="text-[9px] uppercase tracking-[0.14em] font-semibold text-[var(--color-brand-deep)] bg-[var(--color-brand-soft)] px-2 py-0.5 rounded-full">Illustrative</span></p>
              </div>
              <div className="rounded-2xl bg-[var(--color-paper)] border border-[var(--color-rule)] p-5 sm:p-6">
                <p className="text-[11px] uppercase tracking-[0.14em] font-semibold text-[var(--color-mute)] mb-2">HRTO matter</p>
                <p className="text-[26px] sm:text-[28px] font-semibold text-[var(--color-ink)] mb-1">Quoted per file</p>
