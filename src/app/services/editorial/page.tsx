...
export default function EditorialPage() {
  return (
    <>
      ...
      {/* PUBLISHED IN strip — hide bullet separators on mobile, rely on gap-only spacing */}
      <section className="py-10 md:py-14 bg-[var(--color-brand-soft)] border-y border-[var(--color-rule)]">
        <Container size="wide">
          <div className="flex flex-col md:flex-row items-baseline gap-3 md:gap-8">
            <p className="text-[11px] uppercase tracking-[0.18em] font-semibold text-[var(--color-secondary-deep)] flex-shrink-0">Published in</p>
            <div className="flex flex-wrap items-baseline gap-x-4 sm:gap-x-5 gap-y-2 text-[14px] md:text-[17px] text-[var(--color-ink-soft)]">
              {publications.map((p, i) => (
                <span key={p.name} style={{ fontWeight: p.weight, letterSpacing: p.tracking, fontStyle: p.style, textTransform: p.uppercase ? "uppercase" : "none" }}>
                  ...
                  {i < publications.length - 1 && (
                    <span className="hidden md:inline text-[var(--color-brand)] ml-5 not-italic font-normal" aria-hidden="true">·</span>
                  )}
                </span>
              ))}
            </div>
        ...
        <Container size="wide">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-16 items-start">
            <div className="lg:col-span-5">
              ...
              <h3 className="text-[26px] md:text-[32px] leading-[1.15] tracking-[-0.02em] font-semibold text-[var(--color-ink)] mb-6">Per-project, fixed fee.</h3>
              <p className="text-[15px] md:text-[16px] text-[var(--color-mute)] leading-[1.6] mb-6">Every piece is scoped and quoted up front. No hourly meter. Two structural revision rounds included.</p>
              <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1">
                <span className="text-[28px] md:text-[32px] font-semibold text-[var(--color-ink)]">{editorialService.price}</span>
            ...
            <div className="lg:col-span-7 rounded-3xl bg-[var(--color-surface)] border border-[var(--color-rule)] p-6 md:p-10">
              ...
              <p className="text-[16px] md:text-[20px] leading-[1.55] text-[var(--color-ink-soft)] italic">{editorialService.anonymizedCase}</p>
            </div>
          </div>
        </Container>
      </section>
      ...
      {/* FAQ */}
      <section className="py-20 md:py-32 lg:py-40 bg-[var(--color-surface)] border-y border-[var(--color-rule)]">
        <Container>
