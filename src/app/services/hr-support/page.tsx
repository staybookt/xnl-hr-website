        ...
        <Container size="wide">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-5">
            {hrSupportScenarios.map((s, i) => (
              <motion.div key={s.key} initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-30px" }} transition={{ duration: 0.6, delay: i * 0.06, ease: SLOW_OUT }}>
                <Link href={`/contact?topic=${s.key}`} className="group flex flex-col h-full rounded-2xl bg-[var(--color-surface)] border border-[var(--color-rule)] hover:border-[var(--color-slate)]/40 hover:bg-[var(--color-paper)] px-6 py-6 transition-all hover:-translate-y-0.5">
        ...
        <Container size="wide">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-16 items-start">
            <div className="lg:col-span-5">
              ...
              <h3 className="text-[28px] md:text-[32px] leading-[1.15] tracking-[-0.02em] font-semibold text-[var(--color-ink)] mb-6">Project-priced or capped hourly.</h3>
            ...
            <div className="lg:col-span-7 rounded-3xl bg-[var(--color-paper)] border border-[var(--color-rule)] p-8 md:p-10">
              ...
              <p className="text-[18px] md:text-[20px] leading-[1.55] text-[var(--color-ink-soft)] italic">{hrSupportService.anonymizedCase}</p>
            </div>
          </div>
        </Container>
      </section>
