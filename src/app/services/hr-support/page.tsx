...
export default function HRSupportPage() {
  return (
    <>
        ...
        <div className="relative z-10 flex items-center min-h-[88vh] pt-32 md:pt-40 pb-24 md:pb-32">
          <Container size="wide">
            <motion.div initial="hidden" animate="show" variants={STAGGER} className="max-w-5xl">
              ...
              <motion.p
                variants={FADE_UP}
                className="text-[length:var(--text-body-xl)] leading-[var(--text-body-xl--line-height)] text-white/80 max-w-2xl mb-12"
              >
      ...
      {/* SCENARIO TILES (HR support specific) */}
      <section className="py-20 md:py-24 bg-[var(--color-paper)]">
        <Container size="wide">
          ...
          >
            ...
            <h2 className="text-[28px] md:text-[36px] leading-[1.15] tracking-[-0.02em] font-semibold text-[var(--color-ink)]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-5">
            {hrSupportScenarios.map((s, i) => (
              <motion.div
                ...
                transition={{ duration: 0.6, delay: i * 0.06, ease: SLOW_OUT }}
              >
                <Link
                  href={`/contact?topic=${s.key}`}
                  className="group flex flex-col h-full rounded-2xl bg-[var(--color-surface)] border border-[var(--color-rule)] hover:border-[var(--color-slate)]/40 hover:bg-[var(--color-paper)] px-6 py-6 transition-all hover:-translate-y-0.5"
                >
                  <p className="text-[15px] md:text-[16px] font-semibold text-[var(--color-ink)] leading-[1.3] mb-4 flex-1">
                    {s.label}
                  </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-10">
            {hrSupportCovers.map((c, i) => (
              <motion.div
                ...
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true, margin: "-30px" }}
                transition={{ duration: 0.7, delay: i * 0.06, ease: SLOW_OUT }}
                className="border-l-2 border-[var(--color-slate)]/30 pl-6"
              >
        ...
        <Container size="wide">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-16 items-start">
            <div className="lg:col-span-5">
              ...
              <h3 className="text-[28px] md:text-[32px] leading-[1.15] tracking-[-0.02em] font-semibold text-[var(--color-ink)] mb-6">
                Project-priced or capped hourly.
              </h3>
              <p className="text-[16px] text-[var(--color-mute)] leading-[1.6] mb-6">
              ...
              <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1">
                <span className="text-[32px] font-semibold text-[var(--color-ink)]">{hrSupportService.price}</span>
                <span className="text-[14px] text-[var(--color-mute)]">· {hrSupportService.typicalLength}</span>
                <span className="text-[10px] uppercase tracking-[0.14em] font-semibold text-[var(--color-brand-deep)] bg-[var(--color-brand-soft)] px-2 py-0.5 rounded-full ml-2">
                  Illustrative
                </span>
              </div>
            </div>
            <div className="lg:col-span-7 rounded-3xl bg-[var(--color-paper)] border border-[var(--color-rule)] p-8 md:p-10">
              <div className="flex items-center justify-between mb-4">
                <p className="text-[11px] uppercase tracking-[0.14em] font-semibold text-[var(--color-secondary-deep)]">
                  Recent work · anonymized
                </p>
                <span className="text-[10px] uppercase tracking-[0.14em] font-semibold text-[var(--color-brand-deep)] bg-[var(--color-brand-soft)] px-2 py-0.5 rounded-full">
                  Example
                </span>
              </div>
              <p className="text-[18px] md:text-[20px] leading-[1.55] text-[var(--color-ink-soft)] italic">
                {hrSupportService.anonymizedCase}
              </p>
            </div>
          </div>
        </Container>
          ...
          >
            <div className="lg:col-span-4">
              ...
              <h2 className="text-[length:var(--text-display-xl)] leading-[var(--text-display-xl--line-height)] tracking-[var(--text-display-xl--letter-spacing)] font-semibold text-[var(--color-ink)] mb-8">
                What buyers ask on the intake call.
              </h2>
              <Link
                href="/contact"
      ...
      <PageCTA
        ...
        headline={
          <>
            Ready to scope<br />
            <span className="text-brand-gradient">your first project?</span>
          </>
        }
